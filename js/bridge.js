...
class BridgeController {
  ...
  bindEvents() {
    ...
    // Chain selectors
    $("from-chain-display")?.addEventListener("click", () =>
      this.openChainModal("from"),
    );
    $("to-chain-display")?.addEventListener("click", () =>
      this.openChainModal("to"),
    );
    ...
    // Token selectors
    $("bridge-token-display")?.addEventListener("click", () =>
      this.openTokenModal("bridge"),
    );
    $("swap-from-display")?.addEventListener("click", () =>
      this.openTokenModal("swapFrom"),
    );
    $("swap-to-display")?.addEventListener("click", () =>
      this.openTokenModal("swapTo"),
    );
    ...
    // Amount inputs
    $("bridge-input")?.addEventListener("input", () =>
      this.updateBridgeEstimate(),
    );
    ...
    // Execute
    $("execute-bridge-btn")?.addEventListener("click", () =>
      this.executeBridge(),
    );
  ...
  updateSwapDisplay() {
    ...
    if (fromEl)
      fromEl.innerHTML = `<span>${TOKEN_ICONS[this.swapFrom]}</span> ${
        this.swapFrom
      } ▼`;
    if (toEl)
      toEl.innerHTML = `<span>${TOKEN_ICONS[this.swapTo]}</span> ${
        this.swapTo
      } ▼`;
  }
  ...
  updateBridgeEstimate() {
    ...
    if (receive)
      receive.textContent = amount
        ? (amount * 0.999).toFixed(6) + " " + this.bridgeToken
        : "0.00 " + this.bridgeToken;
    if (gas)
      gas.textContent =
        "~" +
        (0.002 + Math.random() * 0.003).toFixed(4) +
        " " +
        CHAINS[this.fromChainId].short;
  }
  ...
  updateSwapEstimate() {
    ...
    if (amount) {
      ...
      setText(
        "swap-rate",
        "1 " + this.swapFrom + " = " + rate.toFixed(4) + " " + this.swapTo,
      );
    } else {
  ...
  openChainModal(target) {
    ...
    container.innerHTML = Object.entries(CHAINS)
      .map(([id, chain]) => {
        const dotClass =
          chain.icon === "🔷"
            ? "sepolia"
        ...
        <div class="chain-option${
          parseInt(id) === curId ? " selected" : ""
        }" data-chain="${id}">
            <span class="chain-icon-lg"><span class="chain-dot ${dotClass}"></span></span>
            <div>
            ...
            ${
              parseInt(id) === curId
                ? '<i class="fa-solid fa-check" style="margin-left:auto;color:var(--green);"></i>'
                : ""
            }
        </div>`;
      })
    ...
    container.querySelectorAll(".chain-option").forEach((el) => {
      el.onclick = () => {
        ...
        if (this.tokenTarget === "bridge") {
          ...
          if (display)
            display.innerHTML = `<span>${TOKEN_ICONS[token]}</span> ${token} ▼`;
  ...
  async executeBridge() {
    ...
    try {
      const tx = await this.contracts.bridgeTokens(
        this.toChainId,
        amount.toString(),
      );
      if (typeof showTx === "function") {
      ...
      if (typeof showTx === "function") {
        showTx(
          "success",
          `Bridged ${amount} ${this.bridgeToken} → ${
            CHAINS[this.toChainId].name
          }!`,
          tx.hash,
        );
      }
  ...
  executeSwap() {
    ...
    setTimeout(() => {
      ...
      if (typeof showTx === "function") {
        showTx(
          "success",
          `Swapped ${amount} ${this.swapFrom} → ${(amount * rate).toFixed(6)} ${
            this.swapTo
          }!`,
          hash,
        );
